...
for ass = 1:size(NumAssets,1)
	...
	for iter_e = 1:size(epsilon,1)
		...
		dirName = "C:/Users/rkannan/Desktop/SA for CCP/portfolio_analytical/" * 
		...
		@variable(m, objvar)
		@variable(m, 0 <= y[1:NumAssets[ass]] <= 1)

		@objective(m, Max, objvar)
		@constraint(m, sum(y[i] for i = 1:NumAssets[ass]) == 1)
		@constraint(m, soc, sum(mean[i]*y[i] for i = 1:NumAssets[ass]) >= objvar + norminvcdf(1-epsilon[iter_e])*norm(sigma*y))
