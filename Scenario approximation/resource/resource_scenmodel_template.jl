...
x_initial_guess = Float64[0.0 0.0 0.0 0.0 43.52912917750612 0.0 0.0 0.0 6.345178399087325e-16 0.0 0.0 25.078984684205707 0.0 0.0 0.0 0.0 15.524911271438539 0.0 0.0 0.0]
x_initial_guess = x_initial_guess'
...
function solveScenarioApproximationModel(numScenarios::Int64,dirName::String)
	...
	while(numViolations > 0)
		...
		mod = Model(solver=IpoptSolver(tol=1e-6,print_level=0))
		
		@variable(mod, objvar)
		@variable(mod, x[i=1:numResources] >= 0,start=x_initial_guess[i])
		@variable(mod, y[1:numResources,1:numCustomers,1:numConstraintsAdded] >= 0)

		@objective(mod, Min, objvar)
		@constraint(mod, sum(costVector[i]*x[i] for i = 1:numResources) <= objvar)
		@constraint(mod, [i=1:numResources, scen=1:numConstraintsAdded], sum(y[i,j,scen] for j = 1:numCustomers) <= rho[i,constraintIndices[scen]]*(x[i])^2)
